...
result %>%
	...
	labs(x = "Shrinkage Values (Lambda)", y = "MSE values")
```
The test MSE for λ equal to 0.01 is 388.9172. Based on the plot above, the MSE appears roughly the same as λ increases. When the value of λ is smaller than 0.01, the models have much larger test MSEs, which means that the model test MSE is insensitive to shrinkage values once they are larger than 0.01.
